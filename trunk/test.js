    ...
    function oembed_callback(response) {
      ...
      h = parseInt(ee.attr("height"));
        ...
        resp.html = '<a href="'+resp.url+'" onclick="$(this).replaceWith(\''+htm+'\');return false;"><div style="width:'+(w-4)+'px;height:'+(h-4)+'px;"></div><b>'+resp.title+(resp.playlist ? '</b> [playlist]' : '</b>')+'<br /><small>by '+resp.author_name+'</small></a>';
      }
